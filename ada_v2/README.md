...
```bash
# Fast local suite (default CI behavior)
python -m pytest -q -m "not integration and not slow" tests

# Run everything (requires optional deps / external services depending on your environment)
...
# Slow-only
python -m pytest -q -m slow tests
```
