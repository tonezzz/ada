...
const socket = (() => {
    ...
    // Using window.location.origin would incorrectly point Socket.IO at the frontend server.
...
function App() {
    ...
    const [isConnected, setIsConnected] = useState(() => !!ipcRenderer); // Power state DEFAULT ON (Electron), OFF (Browser)
    ...
    const startBrowserSTT = () => {
        ...
        const isIOS = /iPhone|iPad|iPod/i.test(ua) || (ua.includes('Mac') && (navigator?.maxTouchPoints || 0) > 1);
        if (isIOS) {
            addMessage('System', 'Speech-to-text is not supported in iPhone Safari/WebView (often shows service-not-allowed). Use the mobile UI (#/mobile) and iOS keyboard mic dictation, or use Chrome/Edge on desktop for STT.');
        ...
        if (!isSecureContext && protocol !== 'https:' && !isLocalhost) {
            addMessage(
                'System',
                'STT blocked: this page is not a secure context (HTTP on a non-localhost domain). In Chrome, microphone/STT may be blocked and show not-allowed. Fix by serving the UI over https, or open via http://localhost:5173 (if available), or in Chrome set chrome://flags/#unsafely-treat-insecure-origin-as-secure and add this origin.'
            );
            isRecognizingRef.current = false;
            setIsMuted(true);
            return;
        ...
        if (!SpeechRecognition) {
            addMessage('System', 'SpeechRecognition not supported in this browser. Use Chrome/Edge.');
            return;
        }
        ...
        if (!speechRecognitionRef.current) {
            ...
            rec.onerror = (event) => {
                ...
                if (err === 'not-allowed') {
                    addMessage('System', 'STT error: not-allowed (microphone permission denied or blocked). In Chrome: click the lock icon in the address bar -> Site settings -> Microphone -> Allow. Also ensure you start STT by clicking the mic button.');
                } else if (err === 'audio-capture') {
                    addMessage('System', 'STT error: audio-capture (no microphone audio captured). On Windows: check Sound settings -> Input device, unplug/replug mic, close Zoom/Teams/Discord, and ensure the mic is not muted in Windows/driver. In Chrome: Settings -> Privacy and security -> Site settings -> Microphone and select the correct device.');
                } else if (err === 'service-not-allowed') {
                    addMessage('System', 'STT error: service-not-allowed (speech service blocked). In Chrome settings, allow speech services / ensure no policy blocks it.');
                } else {
                    addMessage('System', `STT error: ${err}`);
                }
        ...
        const start = async () => {
            try {
                // Proactively request mic permission (SpeechRecognition can fail with not-allowed
                // if permission is denied/blocked or not yet granted).
                ...
                if (name === 'NotAllowedError' || name === 'PermissionDeniedError') {
                    addMessage('System', 'Microphone permission denied. In Chrome: address bar lock icon -> Microphone -> Allow, then reload.');
                } else if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
                    addMessage('System', 'STT start failed: Requested device not found. Chrome cannot find a usable microphone on this Windows machine. Check Windows Sound settings (Input device), plug in/select a mic, close other apps that may be exclusively using it, then reload.');
                } else {
                    addMessage('System', `STT start failed: ${e?.message || e}`);
                }
                isRecognizingRef.current = false;
                setIsMuted(true);
            }
    ...
    const _floatToPcm16Le = (float32Array) => {
    ...
    const _resampleFloat32 = (input, inputSampleRate, targetSampleRate) => {
    ...
    const startBrowserAudioStream = async (deviceId) => {
        await stopBrowserAudioStream();
        if (!navigator?.mediaDevices?.getUserMedia) {
            addMessage('System', 'Microphone capture is not available in this browser (common on iOS in-app browsers). Use typed chat or open the iPhone UI at #/iphone.');
            return;
        }
        ...
        processor.onaudioprocess = (evt) => {
            try {
                ...
                const inputBuf = evt.inputBuffer.getChannelData(0);
                const resampled = _resampleFloat32(inputBuf, ctx.sampleRate, 16000);
                const pcmBuf = _floatToPcm16Le(resampled);
                socket.emit('mic_audio_chunk', pcmBuf);
            } catch (e) {
                // ignore
            }
        };
    ...
    const stopBrowserAudioStream = async () => {
        try {
    ...
    const handLandmarkerInitStartedRef = useRef(false);
    ...
    useEffect(() => {
        ...
        if (ipcRenderer && isConnected && isAuthenticated && socketConnected && micDevices.length > 0 && !hasAutoConnectedRef.current) {
    ...
    useEffect(() => {
        // Socket IO Setup (register once; cleanup on unmount)
        const onConnect = () => {
            setStatus('Connected');
        ...
        if (!handLandmarkerInitStartedRef.current) {
            handLandmarkerInitStartedRef.current = true;
            initHandLandmarker();
        }
    ...
    const addMessage = (sender, textOrMessage) => {
        if (textOrMessage && typeof textOrMessage === 'object') {
            setMessages(prev => [...prev, { sender, time: new Date().toLocaleTimeString(), ...textOrMessage }]);
            return;
        }
        setMessages(prev => [...prev, { sender, text: textOrMessage, time: new Date().toLocaleTimeString() }]);
    };

    const togglePower = () => {
        if (isConnected) {
            socket.timeout(5000).emit('stop_audio', (err, ack) => {
                if (err) {
                    console.warn('[Power] stop_audio ack timeout/error:', err);
                } else {
                    console.log('[Power] stop_audio ack:', ack);
                }
            });
            if (!ipcRenderer) {
            ...
            socket.timeout(10000).emit('start_audio', payload, (err, ack) => {
                if (err) {
                    console.warn('[Power] start_audio ack timeout/error:', err);
                } else {
                    console.log('[Power] start_audio ack:', ack);
                }
            });
            setIsConnected(true);
            setIsMuted(true);
