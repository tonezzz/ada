...
class ProjectManager:
    ...
    def save_image_artifact(self, image_bytes: bytes, prompt: str, mime_type: str = "image/png"):
        """Saves generated image bytes to the project's 'images' folder."""
        if not image_bytes:
            print("[ProjectManager] [ERR] Empty image bytes")
            return None
        ...
        dest_path = self.get_current_project_path() / "images" / filename

        try:
            with open(dest_path, "wb") as f:
                f.write(image_bytes)
            print(f"[ProjectManager] Saved image artifact to: {dest_path}")
            return str(dest_path)
        except Exception as e:
            print(f"[ProjectManager] [ERR] Failed to save image artifact: {e}")
            return None
